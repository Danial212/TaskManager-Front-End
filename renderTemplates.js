...
export {render_mono_category, taskItemTemplate, taskItemTemplate_complete, renderCategories, taskForm, openNewCategory, openEditTask, openNewTask, closeModal, openModal }
...
async function render_mono_category(c, options = {}) {
  ...
  const config = { ...CATEGORY_LAYOUT_CONFIG, ...options };
  ...
  // Determine if this category should take full width or half width
  const isLongName = title.length > config.lengthThreshold;
  const layoutClass = isLongName ? 'col-span-full' : 'col-span-1';
  
  // Truncate title if needed
  const displayTitle = title.length > config.maxDisplayLength 
    ? title.slice(0, config.maxDisplayLength) + config.truncationSuffix 
    : title;
  
  return `
    <button 
      class="group ${layoutClass} relative overflow-hidden
             p-5 rounded-2xl 
             ...
             text-left"
      data-cat="${c.id}"
      aria-label="Category: ${title}, ${taskCount} tasks"
    >
      <div 
        class="absolute inset-0 opacity-0 group-hover:opacity-5 transition-opacity duration-300"
        style="background: linear-gradient(135deg, ${categoryColor}40, ${categoryColor}10);"
        aria-hidden="true"
      ></div>
      ...
      <div class="relative flex items-center gap-4">
        <div class="shrink-0 relative">
          <span 
            class="block w-5 h-5 rounded-xl shadow-lg
            ...
            style="background: ${categoryColor}; box-shadow: 0 0 20px ${categoryColor}40;"
            aria-hidden="true"
          ></span>
          <span 
        ...
        <span 
          class="flex-1 font-semibold text-base text-gray-900 dark:text-white
                 group-hover:text-gray-950 dark:group-hover:text-gray-50
